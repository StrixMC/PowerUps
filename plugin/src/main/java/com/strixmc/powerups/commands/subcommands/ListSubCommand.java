...
public class ListSubCommand implements SubCommand {
    ...
    public ListSubCommand(PowerUpRegistry registry) {
        this.registry = registry;
    }

    @Override
    public void execute(CommandSender sender, String commandLabel, String[] args) {
        if (registry.getPowerUps().isEmpty()) {
            Messages.NO_AVAILABLE.sendMessage(sender);
            return;
        }

        List<String> availableList = registry.getPowerUps().stream().map(powerUp -> (powerUp.isEnabled() ? "&a" : "&c") + powerUp.getID()).collect(Collectors.toList());
